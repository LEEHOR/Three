...
public class CameraTextureView extends TextureView {
    ...
    public void setPicSaveName(String PicName){
     ...
     * 前置摄像头
     */
    private String mCameraIdFace;
    /**
     * 当前是否是前置摄像头
     */
    private boolean isCameraFace = false;
    /**
    ...
    private final ImageReader.OnImageAvailableListener mOnImageAvailableListener
        ...
        public void onImageAvailable(ImageReader reader) {
            mBackgroundHandler.post(new ImageSaver(reader.acquireNextImage(), mFile,PicName));
        }
    ...
    private void setUpCameraOutputs(int width, int height) {
        ...
        try {
            if (manager.getCameraIdList().length>0) {
                //0表示后置摄像头,1表示前置摄像头
                mCameraIdFront = manager.getCameraIdList()[0];
                mCameraIdFace = manager.getCameraIdList()[1];
                //前置摄像头和后置摄像头的参数属性不同，所以这里要做下判断
                if (isCameraFace) {
                    characteristics = manager.getCameraCharacteristics(mCameraIdFace);
                    mCameraId = mCameraIdFace;
                } else {
          ...
          /*  for (String cameraId : manager.getCameraIdList()) {
               ...
               Log.e("rr","maxPreviewWidth宽"+maxPreviewWidth+"/maxPreviewHeight高"+maxPreviewHeight);
               ...
               /* mPreviewSize = Camera2Util.getMinPreSize(map.getOutputSizes(SurfaceTexture.class), maxPreviewWidth, maxPreviewHeight, 1000);*/
                // We fit the aspect ratio of TextureView to the size of preview we picked.
                //根据选中的预览尺寸来调整预览组件（TextureView）的长宽比
               /* int orientation = getResources().getConfiguration().orientation;//获取屏幕方向
                if (orientation == Configuration.ORIENTATION_LANDSCAPE) { //（横向屏幕）
                    this.setAspectRatio(
    ...
    private void lockFocus() {
        try {
            ...
            if (isCameraFace){
                mState=STATE_WAITING_PRECAPTURE; //前置摄像头
            }else {
                mState = STATE_WAITING_LOCK; //后置摄像头手机端
            }
        ...
        try {
            ...
            CameraCaptureSession.CaptureCallback CaptureCallback
                ...
                public void onCaptureCompleted(@NonNull CameraCaptureSession session,
                                               @NonNull CaptureRequest request,
                                               @NonNull TotalCaptureResult result) {
                  //  Log.d(TAG, tFile.toString());
                    mActivity.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            if (tFile!=null){
                                mTackPhotoCallback.tackPhotoSuccess(tFile.toString());
    ...
    private static class ImageSaver implements Runnable {
        ...
        public ImageSaver(Image image, File file,String PicName) {
            mImage = image;
          String systemTime_str = FileUtils.getSystemTime_str();
            File files=new File(file,PicName+"-"+systemTime_str+".jpg");
            Log.e(TAG,"yes/last"+files.toString());
            tFile = files;
        }
        ...
        public void run() {
           ...
           // String systemTime_str = FileUtils.getSystemTime_str();
            //File file=new File(saveFile,"IMG_"+systemTime_str+".netThree");
            try {
